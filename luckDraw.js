...
(function (win) {
  ...
  LuckDraw.prototype._resetVariable = function () {
    ...
    this.span = 5;
    this.startTime = 20;
    this.degAverage = this._keepTwoDecimalFull(
      360 / this.defaultOPts.typeNumber
    );
    this.minDeg =
      this.degAverage * this.defaultOPts.luckNumber +
      this.degAverage / 4 -
      this.degAverage / 2;
    this.maxDeg =
      this.degAverage * (this.defaultOPts.luckNumber + 1) -
      this.degAverage / 4 -
      this.degAverage / 2;
    ...
    this.totalLuckNumber =
      this.defaultOPts.typeNumber * this.defaultOPts.circleNumber +
      (this.defaultOPts.luckNumber + 1);
  ...
  LuckDraw.prototype._turn = function (time, deg) {
    ...
    /* 兼容性处理 */
    this.rotate[0].style[vendor + "TransitionTimingFunction"] =
    ...
    this.rotate[0].style[vendor + "Transform"] =
  ...
  LuckDraw.prototype._changePrize = function () {
    ...
    var resetIndex = function () {
      ...
      _this.rotate[index].className =
        _this.defaultOPts.rotateId.split(".")[1] +
        " " +
        _this.defaultOPts.activeClass;
      _this.index++;

      if (
        _this.index <
        _this.totalLuckNumber - _this.defaultOPts.typeNumber * _this.endCircle
      ) {
        _this.startTime += 1;
        _this.timer = setTimeout(resetIndex, _this.startTime);
      } else if (
        _this.index < _this.totalLuckNumber &&
        _this.index >=
          _this.totalLuckNumber - _this.defaultOPts.typeNumber * _this.endCircle
      ) {
        _this.startTime += 50;
      ...
      for (var r in retn) {
        if (par[i][0] == "#")
          ...
          for (var j = 0; j < tag.length; j++) {
            ...
            if (
              cln &&
              cln.search(new RegExp("\\b" + par[i].replace(".", "") + "\\b")) !=
                -1
            ) {
              _retn.push(tag[j]);
            }
  ...
  LuckDraw.prototype._init = function (opts) {
    ...
    // 从localStorage中获取今天的点击次数，如果没有，则初始化为0
    let todayClicks = localStorage.getItem(today) || "0";
    todayClicks = parseInt(todayClicks, 10); // 将字符串转换为整数
    if (todayClicks >= 1) {
      alert("您今天已经点击过一次了，请明天再来！");
    } else {
      // 如果没有超过，增加点击次数并存储回localStorage
      todayClicks++;
      localStorage.setItem(today, todayClicks.toString());
      if (this.flag) {
        // 扩展defaultOPts对象
        for (var key in opts) {
          this.defaultOPts[key] = opts[key];
